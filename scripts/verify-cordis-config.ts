...
/** Follow a Git symlink placeholder produced by Windows checkouts without symlink support. */
function loadConfigDocument(file: string): unknown {
  const path = resolve(root, file)
  const source = readFileSync(path, 'utf8')
  const linkTarget = source.trim()
  if (!source.includes('\n') && /^(?:\.\.?(?:[\\/]))+.+\.ya?ml$/iu.test(linkTarget)) {
    const target = resolve(dirname(path), linkTarget)
    const targetFromRoot = relative(root, target)
    if (!targetFromRoot.startsWith('..') && existsSync(target)) {
      return loadCordisYaml(readFileSync(target, 'utf8'))
    }
  }
  return loadCordisYaml(source)
}
