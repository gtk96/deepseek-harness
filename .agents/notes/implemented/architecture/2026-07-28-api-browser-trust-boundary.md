...
Two boundaries stay deliberately out of scope: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and this carrier fence remains separate from account authentication. A deployment that needs authenticated remote `/api` Remote calls composes the request-local [Principal authentication layer](2026-08-19-request-local-data-aid-principal-authentication.md); that provider verifies its own gateway boundary and authority resolver, while this fence still supplies browser and DNS-rebinding protection. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
