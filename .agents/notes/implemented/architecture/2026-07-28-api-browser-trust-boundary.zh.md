...
两条边界刻意留在范围之外：可达性由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制，这道载体栅栏仍与账户认证分离。需要认证远程 `/api` Remote 调用的部署组合 request-local [Principal 认证层](2026-08-19-request-local-data-aid-principal-authentication.zh.md)；该 Provider 验证自己的网关边界与权限 resolver，而本栅栏继续提供浏览器与 DNS rebinding 防护。旧守卫的回环 socket 检查被放弃而非泛化：绑定表达可达性、`trustedHosts` 点名远程权威之后，socket 地址提供不了头部栅栏覆盖不到的任何东西。
