...
<a id="ctxmcpclients--mcpclientregistry"></a>

### `ctx.mcpClients` — `McpClientRegistry`

Registry of currently connected MCP clients for direct, non-model raw tool calls. Registrations are disposer-owned: a removed client becomes unavailable immediately.
...
Source: [`packages/mcp/mcp-client/src/mcp-clients.ts`](../../packages/mcp/mcp-client/src/mcp-clients.ts)
