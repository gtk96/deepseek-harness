...
flowchart LR
  ...
  svc_authenticatedPrincipal["ctx.authenticatedPrincipal<br/>Request-local authenticated Principal seam"]
  pkg_authenticated_principal_data_aid["authenticated-principal-data-aid"]
  svc_dataAidTurnPrincipal["ctx.dataAidTurnPrincipal<br/>Trusted Data Aid turn binding"]
  pkg_data_query["data-query"]
  svc_dataQuery["ctx.dataQuery<br/>Controlled semantic query seam"]
  pkg_data_query_dic_be["data-query-dic-be"]
  pkg_mcp_client["mcp-client"]
  svc_mcpClients["ctx.mcpClients<br/>Live raw MCP client registry"]
  pkg_llm["llm"]
  ...
  pkg_authenticated_principal --> svc_authenticatedPrincipal
  pkg_authenticated_principal_data_aid --> svc_authenticatedPrincipal
  pkg_authenticated_principal_data_aid --> svc_dataAidTurnPrincipal
  ...
  pkg_data_query --> svc_dataQuery
  pkg_data_query_dic_be --> svc_dataQuery
  ...
  svc_dataAidTurnPrincipal --> pkg_authenticated_principal_data_aid
  svc_dataQuery --> pkg_authenticated_principal_data_aid
  ...
  svc_mcpClients --> pkg_authenticated_principal_data_aid
...
| `ctx.authenticatedPrincipal` | `seam` | [`authenticated-principal`](../packages/identity/authenticated-principal) | [`authenticated-principal-data-aid`](../packages/identity/authenticated-principal-data-aid) | - | - | Transport providers authenticate one request and establish an asynchronous Principal scope without storing identity in an Agent or Session. |
| `ctx.dataAidTurnPrincipal` | `core` | [`authenticated-principal-data-aid`](../packages/identity/authenticated-principal-data-aid) | - | [`authenticated-principal-data-aid`](../packages/identity/authenticated-principal-data-aid) | - | The dedicated ingress binds one DIC-BE Principal and external turn identity to synchronous Agent message insertion; the model-facing query tool requires that exact binding. |
| `ctx.dataQuery` | `seam` | [`data-query`](../packages/identity/data-query) | [`data-query-dic-be`](../packages/identity/data-query-dic-be) | [`authenticated-principal-data-aid`](../packages/identity/authenticated-principal-data-aid) | - | The runtime selects one available provider and carries only semantic identifiers plus the trusted turn context; providers own governed backend translation. |
| `ctx.mcpClients` | `core` | [`mcp-client`](../packages/mcp/mcp-client) | - | [`authenticated-principal-data-aid`](../packages/identity/authenticated-principal-data-aid) | - | Connected MCP rows publish disposer-owned raw callers for server-side consumers; reconnect replaces registry availability without changing model tool registration. |
