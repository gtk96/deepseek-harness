...
**profile** 是存放在 Harness home 中的具名组装。它列出自己叠放的组合包，存放自己安装的树外插件，并保存用户自己的 `cordis.patch.yml`。`web`、`headless` 和 `data-aid` 作为模板随发行版交付。
...
[`dsh-base`](../packages/bundle/base/README.zh.md) 是 Web 与 headless profile 的通用第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.zh.md) 增加浏览器应用；[`dsh-headless`](../packages/bundle/headless/README.zh.md) 增加一次性运行器，且完全不带服务器。安全专用的 [`dsh-data-aid`](../packages/bundle/data-aid/README.zh.md) profile 改为从自身的封闭 bundle 启动，只包含最小 Agent 栈与受控语义问数路径。
