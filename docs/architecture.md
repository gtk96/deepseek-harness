...
A **profile** is a named composition stored in the Harness home. It lists the bundles it stacks, holds any out-of-tree plugins it installs, and keeps the user's own `cordis.patch.yml`. `web`, `headless`, and `data-aid` ship as templates.
...
[`dsh-base`](../packages/bundle/base/README.md) is the general-purpose first layer for the Web and headless profiles: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all. The security-specialized [`dsh-data-aid`](../packages/bundle/data-aid/README.md) profile starts from its own closed bundle instead, containing only the minimum Agent stack and controlled semantic query path.
