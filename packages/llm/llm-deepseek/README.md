...
| `fillReasoningPlaceholder` | `false` | Add a non-empty `reasoning_content` placeholder to assistant history turns that omitted reasoning |
| `toolResultContinuation` | `false` | Append an execution-only empty user message when a request would end with a tool result |
...
### Wire compatibility options

`fillReasoningPlaceholder` and `toolResultContinuation` are opt-in compatibility controls for gateways that enforce DeepSeek thinking history more strictly than the public endpoint. The former preserves real reasoning verbatim and fills only missing assistant `reasoning_content`; the latter appends an empty user-role continuation only to the serialized request when its last message is a tool result. Neither option changes the durable harness conversation.
