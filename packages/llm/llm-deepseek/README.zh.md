...
| `fillReasoningPlaceholder` | `false` | 为缺少推理的 assistant 历史轮次补充非空 `reasoning_content` 占位文本 |
| `toolResultContinuation` | `false` | 请求将以工具结果结束时，追加仅用于执行的空 user 消息 |
...
`fillReasoningPlaceholder` 与 `toolResultContinuation` 是可选兼容控制，用于比公开端点更严格执行 DeepSeek thinking 历史的网关。前者原样保留真实推理，只为缺失的 assistant `reasoning_content` 填充占位文本；后者只在最后一条消息是工具结果时向序列化请求追加空 user-role continuation。两个选项都不改变持久 harness 对话。
