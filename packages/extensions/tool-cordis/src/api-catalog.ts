...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Service Definition and lifecycle owner for authenticated Principal providers.',
    description: 'Service Definition and lifecycle owner for authenticated Principal providers. The provider subclass implements authenticate; this base class owns only request-local scope and never stores a Principal in a session or Agent.',
    methods: [
      {
        signature: 'abstract authenticate(request: Request, signal: AbortSignal): Promise<AuthenticatedPrincipal>',
        description: 'Authenticate one transport request into a complete Principal.',
        parameters: [{ name: 'request', description: 'standard Fetch request supplied by the transport adapter.' }, { name: 'signal', description: 'request cancellation signal.' }],
        returns: 'the authenticated Principal.',
      },
      {
        signature: 'current(): AuthenticatedPrincipal | undefined',
        description: 'Read the Principal inherited by the current asynchronous operation.',
        parameters: [],
        returns: 'the current Principal, or `undefined` outside an authenticated scope.',
        throws: ['after this service has been disposed.'],
      },
      {
        signature: 'require(): AuthenticatedPrincipal',
        description: 'Read the current Principal and fail when the operation is unauthenticated.',
        parameters: [],
        returns: 'the current authenticated Principal.',
        throws: ['when no Principal is active or this service has been disposed.'],
      },
      {
        signature: 'withPrincipal<T>(principal: AuthenticatedPrincipal | undefined, operation: () => T): T',
        description: 'Run an operation with one exact request-local Principal.',
        parameters: [{ name: 'principal', description: 'Principal to inherit; `undefined` explicitly clears an inherited scope.' }, { name: 'operation', description: 'synchronous or asynchronous operation to invoke.' }],
      ...
      {
        signature: 'withoutPrincipal<T>(operation: () => T): T',
        description: 'Run an operation without inheriting an ambient Principal.',
        parameters: [{ name: 'operation', description: 'synchronous or asynchronous operation to invoke.' }],
        returns: 'the exact value or Promise returned by `operation`.',
      },
    ],
  },
  ...
  },
  {
    ...
    summary: 'Carries a trusted dispatch\'s Principal and business ids into its exact live Agent turn.',
    description: 'Carries a trusted dispatch\'s Principal and business ids into its exact live Agent turn.\n\nA transport adapter must authenticate the dic-be workload, strictly parse the three ids, then call withTurn around synchronous Agent message insertion. The service holds only process-memory references and never writes identity or authorization data to the Session log.',
    methods: [
      {
        signature: 'withTurn<T>(binding: DataAidTrustedTurnBinding, operation: () => T): T',
        description: 'Bind one service-authenticated dic-be dispatch while inserting its Agent message.',
        parameters: [{ name: 'binding', description: 'principal and business ids parsed by the trusted transport adapter, never model arguments.' }, { name: 'operation', description: 'message insertion operation that emits `agent/inbox/inserted` before returning.' }],
        returns: 'the exact value returned by `operation`.',
        throws: ['when any principal or business id is malformed.'],
      },
      {
        signature: 'require(agent: Agent | undefined): DataAidTrustedTurnBinding',
        description: 'Return the complete binding active for this exact Agent.',
        parameters: [{ name: 'agent', description: 'Agent that owns the model tool execution.' }],
  ...
  {
    key: 'mcpClients',
    summary: 'Registry of currently connected MCP clients for direct, non-model raw tool calls.',
    description: 'Registry of currently connected MCP clients for direct, non-model raw tool calls. Registrations are disposer-owned: a removed client becomes unavailable immediately.',
    methods: [
      {
        signature: 'register(serverName: string, caller: McpToolCaller): () => void',
        description: 'Publish one currently connected caller under its configured server name.',
        parameters: [{ name: 'serverName', description: 'configured MCP server namespace.' }, { name: 'caller', description: 'raw invocation function for the connected client.' }],
        returns: 'disposer that removes this exact caller.',
        throws: ['when another live caller already owns the server name.'],
      },
      {
        signature: 'call(request: McpToolCallRequest): Promise<McpToolCallResult>',
        description: 'Invoke a raw tool on the current connected client for one configured server.',
        parameters: [{ name: 'request', description: 'server selection, raw tool name, arguments, and cancellation signal.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'AuthenticatedPrincipalScope',
    declaration: 'export type AuthenticatedPrincipalScope = Readonly<Record<string, unknown>>;',
  },
  {
  ...
  {
    name: 'DataAidClientId',
    declaration: 'export type DataAidClientId = Branded<\'DataAidClientId\'>;',
  },
  {
    name: 'DataAidTrustedTurnBinding',
    declaration: 'export interface DataAidTrustedTurnBinding {\n    readonly principalId: GkUserId;\n    readonly conversationId: DataQueryConversationId;\n    readonly turnId: DataQueryTurnId;\n}',
  },
  {
    name: 'DataOrgCode',
    declaration: 'export type DataOrgCode = Branded<\'DataOrgCode\'>;',
  },
  {
  ...
  {
    name: 'DataRole',
    declaration: 'export type DataRole = Branded<\'DataRole\'>;',
  },
  {
    name: 'DdUserId',
    declaration: 'export type DdUserId = Branded<\'DdUserId\'>;',
  },
  {
  ...
  {
    name: 'GimpStaffId',
    declaration: 'export type GimpStaffId = Branded<\'GimpStaffId\'>;',
  },
  {
    name: 'GkUserId',
    declaration: 'export type GkUserId = Branded<\'GkUserId\'>;',
  },
  {
    name: 'GoalActivation',
  ...
  {
    name: 'McpToolCaller',
    declaration: 'export type McpToolCaller = (request: McpToolCallerRequest) => Promise<McpToolCallResult>;',
  },
  {
    name: 'McpToolCallerRequest',
    declaration: 'export interface McpToolCallerRequest {\n    toolName: string;\n    arguments: Record<string, unknown>;\n    signal: AbortSignal;\n}',
  },
  {
    name: 'McpToolCallRequest',
    declaration: 'export interface McpToolCallRequest extends McpToolCallerRequest {\n    serverName: string;\n}',
  },
  {
    name: 'McpToolCallResult',
    declaration: 'export type McpToolCallResult = Record<string, unknown>;',
  },
  {
    name: 'Message',
  ...
  {
    name: 'TeamCode',
    declaration: 'export type TeamCode = Branded<\'TeamCode\'>;',
  },
  {
    name: 'TeamId',
