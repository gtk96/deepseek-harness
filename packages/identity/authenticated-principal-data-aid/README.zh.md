...
`@deepseek-ai/dsh-authenticated-principal-data-aid/data-query-tool` 在 Data Aid agent preset 中注册模型可见的 `data_query` 能力。模型只能提供语义目录编码——数据集、指标、可选维度、过滤、时间范围、排序与行数上限——绝不提供 SQL、物理标识符或执行配置。工具从 turn 服务解析活动 Principal，并通过 `ctx.dataQuery` 把宿主所有的 `DataQueryRequest` 派发给显式配置的 provider。缺少 turn 身份、Principal 冲突以及 provider 失败都会被拒绝。

生产装配 `@deepseek-ai/dsh-data-query-dic-be` provider，它会签发短期 Principal 断言（`iss`、`aud`、`sub`、`jti`、`iat`、`exp`、轮次绑定）并把语义请求发给独立 broker。broker 是鉴权决策点：它必须验证断言、一次性消费 `jti`、从权威来源解析主体的数据角色，并在服务端执行数据集、表、列、join、谓词与行级授权；DSH 从不解析或授权模型 SQL，因为模型根本不产生 SQL。不得把 provider 指向 `maxcompute-authority.execute_sql` bridge：该 bridge 仅用于认证请求，不能授予业务数据访问权。

随附的 `data-aid` preset 只包含 `data_query`。生产 overlay 要求 broker 端点与断言身份：`DATA_AID_QUERY_BASE_URL`、`DATA_AID_QUERY_PATH`、`DATA_AID_QUERY_ISSUER`、`DATA_AID_QUERY_AUDIENCE`、`DATA_AID_QUERY_ASSERTION_SECRET`，以及有界的 `DATA_AID_QUERY_ASSERTION_TTL_SECONDS`、`DATA_AID_QUERY_TIMEOUT_SECONDS`、`DATA_AID_QUERY_MAX_ROWS`、`DATA_AID_QUERY_MAX_RESULT_CHARS` 和 `DATA_AID_QUERY_DEFAULT_LIMIT`；不得将任何凭据写入源码管理。
