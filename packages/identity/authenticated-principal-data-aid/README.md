...
`@deepseek-ai/dsh-authenticated-principal-data-aid/data-query-tool` registers the model-visible `data_query` capability in the Data Aid agent preset. The model supplies only read-only SQL. The tool derives `ddUserId`, mapped ids, role, teams, organizations, and opaque authorized scope from the active Principal, forwards its cancellation signal to `ctx.mcpClients`, and sends those facts to one dedicated broker. It rejects missing turn identity, conflicting turns, MCP failures, partial responses, non-tabular data, inconsistent counts, results over `maxRows`, and JSON results over `maxResultChars`.

The broker is the authorization decision point. It must enforce read-only SQL and the received identity's table, column, join, predicate, and row authorizations server-side; DSH does not parse or authorize model SQL. Its `authorized_query` response must carry `structuredContent` with `success: true`, `complete: true`, `truncated: false`, matching `rowCount`/`rowsReturned`, string `columns`, and rectangular `rows`. Do not point this tool at the `maxcompute-authority.execute_sql` bridge: that bridge exists only to authenticate a request and cannot grant business-data access.

The shipped `data-aid` preset contains only `data_query`. The production overlay requires a separate `DATA_AID_QUERY_MCP_URL` and `DATA_AID_QUERY_MCP_TOKEN`, keeps both MCP bridges `exposeTools: false`, and selects that preset by default. `DATA_AID_QUERY_PROJECT`, `DATA_AID_QUERY_MAX_CU`, `DATA_AID_QUERY_TIMEOUT_SECONDS`, `DATA_AID_QUERY_MAX_ROWS`, and `DATA_AID_QUERY_MAX_RESULT_CHARS` are required deployment configuration; no credential belongs in source control.
