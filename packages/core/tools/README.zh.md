...
统一 schema DSL 使用 `ParameterSchemaSpec` 表示参数对象，使用 `ValueSchemaSpec` 表示任意 JSON 值根。它支持 `string`、`number`、`integer`、`boolean`、`null`、`array`、`object`、仅供作者使用的 `json` 与恰好匹配一个分支的 `oneOf`；标量 `enum` 与 `const` 值保持类型正确。字符串、数值与数组约束会进入原生 schema 并在运行时执行，不改变生成的静态类型。每个显式 DSL 对象都声明 `additionalProperties` 策略，工具可用 `parameterRoot: { additionalProperties: false }` 关闭参数根。`InferValue` 在 16 层容器内保留精确类型，之后加宽为 `JsonValue`。

原始 JSON Schema（`JsonSchemaNode`）是工具输出、subagent、工作流与 MCP 共享的协议级对应类型。它允许任意 JSON 根、对象属性与开放性、数组成员与数量约束、字符串长度与 pattern 约束、包含边界的数值范围、标量 `enum` 与 `const`，以及恰好匹配一个分支的 `oneOf`。`assertSupportedJsonSchema()` 拒绝不受支持或类型位置错误的构造，`validateJsonSchemaValue()` 返回带路径的违规信息。要求对象根的调用方使用 `assertObjectJsonSchema()`，而不是收窄共享词汇。
