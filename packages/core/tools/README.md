...
The unified schema DSL uses `ParameterSchemaSpec` for the parameter object and `ValueSchemaSpec` for any JSON-value root. It supports `string`, `number`, `integer`, `boolean`, `null`, `array`, `object`, author-only `json`, and exact-one `oneOf`; scalar `enum` and `const` values are type-correct. String, numeric, and array constraints are emitted into native schemas and enforced at runtime without changing the generated static type. Every explicit DSL object declares its `additionalProperties` policy, and a tool may close the parameter root with `parameterRoot: { additionalProperties: false }`. `InferValue` preserves exact types through 16 container levels before widening to `JsonValue`.

A raw JSON Schema (`JsonSchemaNode`) is the wire-level counterpart shared with tool outputs, subagents, workflows, and MCP. It permits any JSON root, object properties and openness, array item and count constraints, string length and pattern constraints, inclusive numeric ranges, scalar `enum` and `const`, and exact-one `oneOf`. `assertSupportedJsonSchema()` rejects unsupported or type-misplaced constructs, while `validateJsonSchemaValue()` returns path-qualified violations. Callers that require an object root use `assertObjectJsonSchema()` rather than narrowing the shared vocabulary.
