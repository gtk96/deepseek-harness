...
| `exposeTools` | `true` | Publish discovered tools to `ctx.tools`; set `false` for direct-only consumers |
...
Mount `@deepseek-ai/dsh-mcp-client/mcp-clients` before each bridge instance. A non-model consumer calls the currently connected server through `ctx.mcpClients.call({ serverName, toolName, arguments, signal })`; the registry forwards the raw tool name, caller cancellation, and the server's configured timeout, then returns the unvalidated MCP response for the consumer to validate. Set `exposeTools: false` when the server is direct-only so discovery never publishes its definitions to `ctx.tools` or model context. Calls reject while the server is disconnected, after retry exhaustion, or after disposal.
...
| [`src/mcp-clients.ts`](src/mcp-clients.ts) | Live raw-call registry for direct, non-model consumers |
