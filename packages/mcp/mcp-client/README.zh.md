...
| `exposeTools` | `true` | 将发现的工具发布到 `ctx.tools`；仅供直接消费方使用时设为 `false` |
...
在每个 bridge 实例之前挂载 `@deepseek-ai/dsh-mcp-client/mcp-clients`。非模型消费方通过 `ctx.mcpClients.call({ serverName, toolName, arguments, signal })` 调用当前已连接服务器；注册表会转发原始工具名、调用方取消信号与该服务器配置的超时，并返回未校验的 MCP 响应供消费方自行校验。服务器仅供直接消费方使用时设置 `exposeTools: false`，这样发现结果不会发布到 `ctx.tools` 或模型上下文。服务器断开、重试耗尽或插件 dispose 后，调用会拒绝。
...
| [`src/mcp-clients.ts`](src/mcp-clients.ts) | 面向直接非模型消费方的活动原始调用注册表 |
